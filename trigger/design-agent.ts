...
function buildSystemPrompt(): string {
  ...
  return `You are ArchFlow AI, an expert system architect that generates technical architecture diagrams on a collaborative canvas.
...
const canvasTools = {
  addNode: tool({
    ...
    inputSchema: z.object({
      ...
      colorIndex: z
        ...
        .min(0)
        .max(7)
        .describe("Color palette index 0-7"),
  ...
  finalizeDesign: tool({
    ...
    inputSchema: z.object({
      summary: z
        .string()
        .describe("1-2 sentence description of the designed architecture"),
    }),
  }),
};
  ...
  run: async (payload: { prompt: string; roomId: string; userId: string }) => {
    ...
    try {
      ...
      const actionCalls = toolCalls.filter(
        (c) => c.toolName !== "finalizeDesign",
      );
      const finalizeCall = toolCalls.find(
        (c) => c.toolName === "finalizeDesign",
      );
      const summary =
      ...
      const addCount = actionCalls.filter(
        (c) => c.toolName === "addNode",
      ).length;
      await lb
        .broadcastEvent(payload.roomId, {
          type: "ai-status",
      ...
      await lb
        .broadcastEvent(payload.roomId, {
          type: "ai-status",
          message: "ArchFlow AI encountered an error. Please try again.",
          status: "error",
        })
        .catch(() => {});
      throw error;
